  ...
  static loadStats() {
    ...
    fs.readdir(this.jsonsFolder, (err, files) => {
      files.forEach((file) => {
        ...
        const rollUser: RollUser | undefined = UsersManager.getUser(
          loadedUser.userId,
        );
        if (rollUser) {
          loadedUser.stats.forEach((stat) =>
            stat.rolls.forEach((roll) =>
              rollUser.addRoll(new DiceResult(new Dice(stat.dice), roll)),
            ),
          );
        }
      });
