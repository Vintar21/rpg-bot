...
import { Dice, DiceResult, DiceUtils } from '../utils/dice-utils';
import {
  bold,
  emptyLine,
  indent,
...
export class LuckCommand implements Command {
  ...
  private getDefaultAnswer(
    ...
    answer += line(
      indent(underline(`Modifier :`) + ` +/-${modifier.result} of luck`),
    );

    return answer;
  }

  private getCheckedAnswer(
    passed: boolean,
    luckCheck: DiceResult,
    modifier: DiceResult,
  ): string {
  ...
  async run(parsedUserCommand: CommandContext): Promise<void> {
    ...
    if (this.isValidArg(parsedUserCommand.args)) {
      const luckValue: boolean =
        luckCheck.result <= parseInt(parsedUserCommand.args[0]);
      answer =
        mentionUser(parsedUserCommand) +
        emptyLine() +
        this.getCheckedAnswer(luckValue, luckCheck, modifier);
    } else {
      answer =
        mentionUser(parsedUserCommand) +
        emptyLine() +
        this.getDefaultAnswer(luckCheck, modifier);
    }
